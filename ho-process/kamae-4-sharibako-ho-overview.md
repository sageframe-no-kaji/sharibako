...
### ho-06.7 — Delete: the missing destructive verb (closed 2026-07-12)

- **COMPLETED** — ho-06.7 authored, executed, and gated same-day (branch `delete-verb`, stacked on `panel-chrome`). Four destructive verbs where there were none: `deleteScope`, `deleteSecret`, `deleteSharedEntry` — all filesystem-only on the keyless `VaultCore(vaultURL:)` (no age, no Touch ID), in a new `VaultCore+Deletion.swift`; shared-delete guarded by `linkGraph` (`sharedEntryLinked` names the linkers; `--force` orphans them). CLI: `sharibako delete <scope>` / `<scope> <key>` / `--shared`, confirmation like `clean` (TTY-gated, exit 130 on decline). GUI: rust-toned Delete Scope (action panel) + Delete Secret (detail pane) — destructive-rust's first real in-window consumers — each staging a pending deletion behind a system-rendered confirm dialog (dialogs untintable, per 06.5). Blast radius = vault only (markers + materialized `.env` left in place, git retains/recoverable, `sync` commits). Per-key delete was a mid-build scope widening (Decision 7) after dogfooding. 759 tests (+28), warnings-as-errors + both linters clean; view files stay CI-excluded. Gate passed on the signed `/Applications` build (rust reads right on both buttons, dialogs acceptable, deletion feels safe).
- **NEXT** — ho-06.3 (first-run + age key + backup nudge + GUI ingest — the Checkpoint-2 gate).
- **ACTION ITEMS / BLOCKS** — none blocking. GUI **delete-shared** deferred to ho-07 (no shared-entry surface in the Workshop yet); Core + CLI carry it. Branch `delete-verb` stacked on unmerged `panel-chrome`/PR #15 — rebase onto `main` when #15 merges; push + PR are the practitioner's ritual, pending. Still owed: ho-06.3 (NEXT), ho-06.6, the CLI ho (scriptable `init`, `git init`, non-atomic `ingest`), unlinked-markers UI verification.
- **PROJECT LIFECYCLE** — dev
