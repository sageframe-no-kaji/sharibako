...
- **Only an uncatchable `SIGKILL` can leave the temporary age-key file behind.** On macOS the Keychain-retrieved key is written to a `0600` file in the per-user temp directory for the duration of one operation. Cleanup runs on normal completion, on thrown errors, and now — for the life of the temp file — on `SIGINT`/`SIGTERM`/`SIGHUP`/`SIGQUIT`: a trap scrubs and deletes the file, then re-raises the signal so the process still dies as it would have (Ctrl-C during a Touch ID prompt no longer leaks). The single residual is `SIGKILL` (`kill -9`), which the kernel delivers without giving any process a chance to run cleanup — uncatchable by design. The file is unreadable by other users and macOS sweeps the temp directory periodically.
...
- **`age` and `git` are resolved via `PATH` first.** Sharibako searches your `PATH` before its built-in fallback directories, so a `PATH` you control decides which `age`/`git` runs. A poisoned `PATH` could serve a hostile binary — but anyone who can edit your `PATH` can already alias `sharibako` itself, so this widens no boundary that was not already open.
