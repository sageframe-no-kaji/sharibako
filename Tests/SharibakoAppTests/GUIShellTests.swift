...
/// Mirrors `CLIShellTests` (`Tests/SharibakoCLITests/CLIShellTests.swift`) —
/// `GUIShell` is the Workshop's own copy of the PATH-plus-fallback lookup
/// (ho-06.3 Decision 5), so its tests mirror the CLI's mirror.
@Suite("GUIShell")
struct GUIShellTests {
    ...
    func findExecutableMissingThrows() {
        ...
        let error = #expect(throws: VaultError.self) {
            _ = try GUIShell.findExecutable(bogus)
        }
    ...
    func findExecutableHonorsPath() throws {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("guishell-path-\(UUID().uuidString)")
        ...
        let onPath = try GUIShell.findExecutable("git", pathVariable: dir.path)
        ...
        let fallback = try GUIShell.findExecutable("git", pathVariable: "")
    ...
    func runCapturesStreamsAndExitCode() throws {
        ...
        let result = try GUIShell.run(sh, ["-c", "printf out; printf err 1>&2; exit 3"])
    ...
    func runSuccessExitCode() throws {
        let sh = URL(fileURLWithPath: "/bin/sh")
        let result = try GUIShell.run(sh, ["-c", "exit 0"])
